...
GOGS_USER_COUNT=${ARG_GOGS_USER_COUNT:-1}
...
OPENSHIFT_APPS_HOSTNAME=apps.cicdopenshift.redhat-tlv.com
...
echo "GOGS_HOSTNAME=$GOGS_HOSTNAME"
...
function deploy_nexus() {
  oc process -f https://raw.githubusercontent.com/azipory/nexus/master/nexus2-persistent-template.yaml \
    -n $INFRA_PROJECT | oc create -f - -n $INFRA_PROJECT
...
#  local _GITHUB_REPO_NAME=devops-workshop-labs
  ...
  for i in `seq 100 $GOGS_USER_COUNT`; do
